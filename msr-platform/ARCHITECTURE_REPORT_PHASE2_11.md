...
`sts:GetCallerIdentity`, que usa la verificación de arranque, **no aparece** en la política
porque AWS lo autoriza siempre a cualquier identidad: no hace falta concederlo.

El JSON literal de la trust policy y de la política, los comandos de la CLI y los criterios de
aceptación están en `AWS_BOOTSTRAP_REQUEST.md`, el entregable para el equipo de Cloud/IAM. Los
dos documentos los publica ahora Terraform en el output `backend_external_role_bootstrap`, con
test propio (`tests/external_backend_identity.tftest.hcl`) que fija el `sub` verificado, la
ausencia de comodines y el alcance del statement de DynamoDB.
...
Configuración única en Devin (el repositorio no tenía blueprint). **Aprobado por el usuario**:
...
La acción sólo escribe ficheros: no llama a STS al construir el snapshot, así que el blueprint
no falla porque el rol todavía no exista (verificado leyendo su implementación). Hasta que el
bootstrap esté aplicado, `scripts/start_poc.sh` instala las dependencias por sí mismo y arranca
en modo `mock` — la demo funciona simulada.
...
6. **Laboratorio, comprobación no destructiva**: contra AWS, `python -m app.preflight`
   (identidad federada, valores por defecto seguros, alcanzabilidad de la tabla de locks con
   `GetItem` y estado del laboratorio con el reset **prohibido**, `allow_reset=False`); en mock,
   `python -m app.lab_hook` sin `--confirm`. Ninguno resetea. Si el laboratorio no está listo,
   lo dice y deja la aplicación arriba.
...
`.env`, ni nombres de runbooks, ni ejecutar ningún comando de la CLI de AWS (el helper del
`credential_process` sí la invoca por dentro, y ya viene instalada en la VM). Sólo pedirle a
Devin que arranque la PoC.

Playbook/knowledge asociado: `msr-platform/PLAYBOOK_START_POC.md` y el playbook «Arrancar la
PoC de MSR» ya creado en Devin, para que «arranca la PoC de MSR» sea suficiente.
...
Tests que lo fijan (`backend/tests/test_session_startup.py` y `test_preflight.py`): el
preflight exige credenciales temporales y federadas, rechaza cualquier clave estática visible
para el SDK, lee la tabla de locks sin escribirla y llama al laboratorio siempre con
`allow_reset=False` (un laboratorio ya parcheado **falla** la comprobación en vez de
recrearse). Además, el script no contiene ninguna
...
| Blueprint con `setup-aws-oidc` y playbook | **Aprobados** por el usuario |
| Preflight de validación en dry-run | Implementado (`python -m app.preflight`) |
| Paquete atómico para el equipo de Cloud/IAM | Entregado: `AWS_BOOTSTRAP_REQUEST.md` |
| Proveedor OIDC, `MSRExternalBackendRole`, tabla de locks | **Pendientes del equipo de cloud** (ninguna mutación hecha desde aquí) |
| Arranque en modo `aws` verificado de punta a punta | Pendiente: requiere el bootstrap anterior, en una sesión nueva |
| Patch/reset reales | Pendientes de autorización explícita y separada |

La validación end-to-end en dry-run (14 comprobaciones, sin patch ni reset) se ejecutará en una
sesión nueva cuando el equipo de cloud confirme el bootstrap: checklist en
`AWS_BOOTSTRAP_REQUEST.md` §5.
