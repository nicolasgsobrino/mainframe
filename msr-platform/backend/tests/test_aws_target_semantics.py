...
def test_ring_with_several_real_instances_is_rejected(aws_store):
    ...
    # Sin objetivo lógico concreto, el ID de sandbox se aplica a todos los activos.
    aws_store.settings.sandbox_instance_id = INSTANCE
    aws_store.settings.sandbox_logical_target_id = ""
...
def test_exactly_one_real_instance_is_accepted(aws_store):
    ...
    logical_ids = ring_logical_ids(aws_store, tid, ring_no)
    aws_store.settings.sandbox_instance_id = INSTANCE
    aws_store.settings.sandbox_logical_target_id = logical_ids[0]
...
def test_only_track_a_reaches_the_aws_provider(aws_store, track):
    ...
    ring_no = next_ring(aws_store, tid)
    aws_store.settings.sandbox_instance_id = INSTANCE
    aws_store.settings.sandbox_logical_target_id = ring_logical_ids(aws_store, tid, ring_no)[0]
    aws_store.preapprove_ring(tid, ring_no)
